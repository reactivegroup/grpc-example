...
public class Http2Ping {
   ...
   */
  public void addCallback(final PingCallback callback, Executor executor) {
    Runnable runnable;
    synchronized (this) {
      if (!completed) {
  ...
  public boolean complete() {
    Map<PingCallback, Executor> callbacks;
    ...
    for (Map.Entry<PingCallback, Executor> entry : callbacks.entrySet()) {
   ...
   * duration.
   */
  private static Runnable asRunnable(final PingCallback callback,
